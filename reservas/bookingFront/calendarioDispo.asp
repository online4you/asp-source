...
disponiblesStyle="color:#006633; font-weight: bold"
noDisponiblesStyle="color:#FF0000; font-weight: bold"
hotelCerradoStyle="color:#000000; font-weight: bold"

'Variables post

idh=paClng(request.QueryString("idh"))
if (idh=0 and  request.QueryString("hg_idh")<>"") then
	...
	cs="SELECT CodigoEsta FROM " & precrs & "Establecimientos esta where esta.hg_CodigoEsta=" & request.QueryString("hg_idh")
	'response.write cs
	rs.open cs,base
	if not rs.eof then
		crsId=rs("CodigoEsta")
...
if mesini=0 then 'poner por defecto la fecha del día
	fechaRef=date
	if (request.QueryString("fechaRef")<>"") then
		fechaRef=cdate(request.QueryString("fechaRef"))
	end if
	mesini=month(fechaRef)
	anyini=year(fechaRef)
end if
...
<table width='100%' class='table-calendario' align="center" cellpadding="0">
	...
	<td colspan="2">
		<table width='100%' border='0'>
			<tr>
				<td class='title-style' align="left">
					<div id='spanLeft' class='title-style' onclick='javascript:dM()'><img src="css/img/prevlabelAdjusted.gif"/></div>
				</td>
				<td class='title-style' align="left">
					<div style="float:left; margin-left:20px;" id='listaHabitaciones'>
						...
						<div class='capa_lista'>
							<span id='selectMarca' class='titulo_listaNaranja'><%=nombreHabi%></span>
				...
				<td class='title-style' align="left">
					<div id='spanRight' class='title-style' onclick='iM()' style="float:right"><img src="css/img/nextlabelAdjusted.gif"/></div>
				</td>
			</tr>
		</table>
	...
	<div class='content'>
		...
		<tr><td class='title-name-day' align='center' colspan="7" style="background-color:#F38A12"><%=ucase(nombreMes(mesini)) & " " & anyini%></td></tr>
		<tr>
		<%for d=1 to 7%>
			<td width='27' class='title-name-day' align='center' style="background-color:#F38A12"><%=diasCortos(d)%></td>
		<%next%>
		</tr>
		<tr>
		<%colu=1
		for d=1 to 31
			...
			if ndia=-1 then 'no hay resultados%>
				<td align="center">
					<span style="<%=hotelCerradoStyle%>"><%=d%></span>
				</td>
				<%colu=colu+1
				if colu>7 then
				...
				for dd=d-1 to ubound(RegDias,2)%>
					<td align="center"<%if RegDias(DDispo,dd)=0 then response.write " style='"&noDisponiblesStyle&"'" else response.write " style='"&disponiblesStyle&"'"%>><%=dd+1%><%'response.write "[" & RegDias(DDispo,dd) & "]"%></td>
					<%colu=colu+1
					if colu>7 then
						response.write "</tr><tr>"
						colu=1
	...
	<td class='body-style' valign="top">
		<div class='content'>
		<table border=0 class='body-style' width="100%">
		<tr><td class='title-name-day' align='center' colspan="7" style="background-color:#F38A12"><%=ucase(nombreMes(mesfin)) & " " & anyfin%></td></tr>
		<tr>
		<%for d=1 to 7%>
			<td width='27' class='title-name-day' align='center' style="background-color:#F38A12"><%=diasCortos(d)%></td>
		<%next%>
		...
		for d=1 to 31
			...
			if ndiaF=-1 then 'no hay resultados%>
				...
				for dd=d-1 to ubound(RegDiasF,2)%>
					<td align="center"<%if RegDiasF(DDispo,dd)=0 then response.write " style='"&noDisponiblesStyle&"'" else response.write " style='"&disponiblesStyle&"'"%>><%=dd+1%><%'response.write "[" & RegDiasF(DDispo,dd) & "]"%></td>
					<%colu=colu+1
					if colu>7 then
						response.write "</tr><tr>"
						colu=1
	...
	<tr><td colspan="2" align="left" bgcolor="#FFFFFF" style="padding-left:5px;">
		<p style="<%=disponiblesStyle%>">X - <%=objIdioma.getTraduccionHTML("i_disponibles")%></p>
		<p style="<%=noDisponiblesStyle%>">X - <%=objIdioma.getTraduccionHTML("i_nodisponibles")%></p>
		<p style="<%=hotelCerradoStyle%>">X - <%=objIdioma.getTraduccionHTML("i_hotelcerrado")%></p>
	</td></tr>
</table>
</div>
</form>
