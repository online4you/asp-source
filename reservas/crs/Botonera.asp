...
<div id="botonera">
	...
	<div id='dentroBotonera'>
		...
		<a href='javascript:openReport("reservasBDL","bdl_timcre","DESC");'>
		<div class='fila_menu'><%=objIdioma.getTraduccionHTML("i_listareservas")%> BDL</div></a>
		<a href='javascript:openReport("reservasPortVill","hl_timcre","DESC");'>
		<div class='fila_menu'><%=objIdioma.getTraduccionHTML("i_listareservas")%> PortVill</div></a>
		...
		<div class="titulo_menu"><%=ucase(objIdioma.getTraduccionHTML("i_modificar"))%> PortVill</div>
		<a href='javascript:actualizaHotelan();'>
		<div class='fila_menu'><%=objIdioma.getTraduccionHTML("i_modificar")%> PortVill</div>
		</a>
		<br>
		<%
...
<script type="text/javascript">
	function guardaLinuxAjax(){
		...
		$j.ajax({ 
			...
			success: function(response, textStatus, jqXHR){ 
				...
				alert("Operación llevada a cabo con éxito:\n\n"+response);
			}, 
			// callback handler that will be called on error 
			error: function(jqXHR, textStatus, errorThrown){ 
				alert(textStatus + " - Error en la operación. Recupere el backup y guardelo en lugar seguro!!!!");
			}, 
	...
	function guardaLinux(){
		if(confirm("Esta operación puede durar unos minutos. No cierre el explorador ¿Quiere continuar?")==true){       
	...
	function actualizaHotelan(){
		if(confirm("Esta operación puede hacer que la disponibilidad de portVill se vea afectada durante el proceso de carga. ¿Quiere continuar?")==true){       
		   var str = "<form name='f1' method='post' target='_self' action='/PCRS/apps/portvill/actualizarDatos.html'>";
				str += "<input type='hidden' ";
				str += "name='pass' ";
				str += "value='Javier69'>";
				str += "<br>\n";
	...
	function openReport(carpeta,orderBy,sort){
		  var server="<%=split(Request.ServerVariables("SERVER_NAME"),".")(1)%>.<%=split(Request.ServerVariables("SERVER_NAME"),".")(2)%>";	  
		   var str = "<form name='f1' method='get' target='_self' action='http://reports."+server+"/"+carpeta+"/index.php'>";
				str += "<input type='hidden' ";
				str += "name='orderby' ";
				str += "value='"+orderBy+"'>";
				str += "<input type='hidden' ";
